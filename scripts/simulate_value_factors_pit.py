#!/usr/bin/env python3
"""
가치 버킷 팩터 탐색 시뮬 — PIT(16분기) 교정판.

simulate_growth_factors_pit.py와 동일한 구조, 이벤트만 value_pit16=1로 필터.
목적: signal_value()가 signal_growth()와 같은 공식(rev2y가속+P/OP저평가+capex1y↓)을
그대로 쓰고 있는데, value 버킷 자체 그리드에서도 이 조합이 최선인지 확인.
"""
...
def load_pit_value_panel() -> pd.DataFrame:
    ...
    pit = pd.read_sql("SELECT ticker, anchor_term, value_pit16 FROM pit_buckets16", con)
    con.close()

    events = panel.merge(pit, on=['ticker', 'anchor_term'], how='inner')
    events = events[events['value_pit16'] == 1].copy()
...
def main():
    df = load_pit_value_panel()

    today = datetime(2026, 7, 22)
    c12 = today - timedelta(days=366)
    c18 = today - timedelta(days=549)

    df12 = df[(df['filing_anchor_date'] <= c12) & df['alpha_12m'].notna()].copy()
    df18 = df[(df['filing_anchor_date'] <= c18) & df['alpha_18m'].notna()].copy()

    base_n   = len(df12)
    base_a12 = df12['alpha_12m'].mean() * 100
    base_a18 = df18['alpha_18m'].mean() * 100
    print(f"PIT value 이벤트: {df['ticker'].nunique()}종목  전체 {len(df):,}행")
    print(f"12m 완전 구간: {len(df12):,}행 (anchor ≤ {c12.date()})")
    print(f"18m 완전 구간: {len(df18):,}행 (anchor ≤ {c18.date()})")
