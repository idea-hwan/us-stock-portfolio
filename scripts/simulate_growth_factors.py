...
FACTORS = [
    ...
    ('op_dec',     lambda r: _pos(r,'op_geom_1y_mcum') and _v(r,'op_geom_1y_mcum') < _v(r,'op_geom_2y_mcum') if _v(r,'op_geom_2y_mcum') else False, '성장'),
...
def stats(df12, df18, mask12, mask18=None, base_n=None):
    g12 = df12[mask12]
    n   = len(g12)
    if n < 10:
        return None
    a12 = g12['alpha_12m'].mean() * 100
    freq = n / (base_n or len(df12)) * 100
...
def main():
    ...
    print(f"성장 버킷: {df12['ticker'].nunique()}종목  관측 {base_n:,}행")
