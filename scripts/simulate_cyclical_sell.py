...
"""
사이클 버킷 매도 신호 시뮬
세 축: 성장(op/rev 고점·둔화), 고평가(P/E·P/OP·P/S·P/FCF), 투자(capex급증)
단일 → 2팩터(축 혼합) → 3팩터(세 축) 조합 탐색
"""
...
def _low(r, cur, avg):
    c, a = _v(r, cur), _v(r, avg)
    return c is not None and a is not None and a > 0 and c / a < 1 / T_HIGH


# ── 팩터 정의 ─────────────────────────────────────────────────────────────────

FACTORS = [
    # ── 성장 (이익/매출 고점·둔화 = 매도 후보) ──
    ('op_1y↑',     lambda r: _pos(r, 'op_geom_1y_mcum'),                    '성장'),
    ('op_2y↑',     lambda r: _pos(r, 'op_geom_2y_mcum'),                    '성장'),
    ('op_acc',     lambda r: _acc(r, 'op_geom_1y_mcum', 'op_geom_2y_mcum'), '성장'),
    ('op_dec',     lambda r: _dec(r, 'op_geom_1y_mcum', 'op_geom_2y_mcum'), '성장'),
    ('op_2y_acc',  lambda r: _acc(r, 'op_geom_2y_mcum', 'op_geom_4y_mcum'), '성장'),
    ...
    ('pe_low',     lambda r: _low(r,  'pe_20d',   'pe_4y'),   '고평가'),
    ('pop_low',    lambda r: _low(r,  'pop_20d',  'pop_4y'),  '고평가'),
    # ── 투자 (capex 급증 = 미래 FCF 압박) ──
...
def main():
    ...
    snap = snap[snap['bucket'].str.contains('cyclical')].copy()
    ...
    base_n = len(df)
    b = {'a3': df['alpha_3m'].mean()*100, 'a6': df['alpha_6m'].mean()*100, 'a9': df['alpha_9m'].mean()*100}

    print(f"사이클 버킷: {df['ticker'].nunique()}종목  관측 {base_n:,}행")
    print(f"베이스라인:  3m={b['a3']:+.1f}%  6m={b['a6']:+.1f}%  9m={b['a9']:+.1f}%")

    # ── 1. 단일 팩터 ─────────────────────────────────────────────────────────
    print()
    print("=" * 74)
    print("1. 단일 팩터 (n≥10, 9m alpha 기준)")
    print("=" * 74)
    singles = []
    for lbl, fn, axis in FACTORS:
        mask = apply_f(df, lbl)
        st = stats(df, mask, base_n)
        if st:
    ...
    # ── 2. 2팩터 조합 (축 다르게, 베이스 -2%p 이상 underperform) ─────────────
    print()
    print("=" * 74)
    ...
    # ── 3. 3팩터 조합 (세 축 모두, n≥10, 9m 베이스 -3%p 이상 under) ─────────
    print()
    print("=" * 74)
    print("3. 3팩터 조합 (성장+고평가+투자, n≥10, 9m 베이스 -3%p 이상 under)")
    print("=" * 74)
    ...
    print()
    print("=" * 74)
    print("4. 업종별 베이스 × 주요 고평가 팩터")
    print("=" * 74)
    VAL_FACTORS = [f for f in FACTORS if f[2] == '고평가']
    for ctype in sorted(df['cyclical_type'].dropna().unique()):
        sub = df[df['cyclical_type'] == ctype]
        sb  = {'a3': sub['alpha_3m'].mean()*100, 'a6': sub['alpha_6m'].mean()*100, 'a9': sub['alpha_9m'].mean()*100}
        print(f"\n  [{ctype}]  n={len(sub)}  베이스 3m={sb['a3']:+.1f}%  6m={sb['a6']:+.1f}%  9m={sb['a9']:+.1f}%")
        for lbl, fn, _ in VAL_FACTORS:
            mask = sub.apply(fn, axis=1)
            st = stats(sub, mask, len(sub))
            if st:
                print(f"    {lbl:13s}  {fmt(st, sb)}")
