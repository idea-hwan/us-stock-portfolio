"""
종목 분류 (독립 라벨, 중복 허용) → data/analytics/all_stocks.csv (종목당 한 행, bucket 컬럼에 라벨)
...
SNAPSHOT_COLS = [
    'ticker', 'bucket', 'anchor_term', 'filing_anchor_date',
    'adj_close_anchor',
...
def classify(df_ttm: pd.DataFrame, df_growth: pd.DataFrame,
             as_of: str | None = None) -> dict[str, list[str]]:
    """ticker → labels 매핑. labels는 복수 허용 (예: ['growth','value']), 둘 다 아니면 빈 리스트.
...
def main():
    ...
    bucket_map = classify(df_ttm, df_growth, as_of)

    # 집계 출력
    from collections import Counter
    label_cnt: Counter = Counter()
    unclassified_cnt = 0
    for labels in bucket_map.values():
        if not labels:
            unclassified_cnt += 1
        for lb in labels:
            label_cnt[lb] += 1

    for b in ['growth', 'value']:
        print(f"  {b:15}: {label_cnt.get(b, 0):3}개")
    print(f"  {'(미분류)':15}: {unclassified_cnt:3}개")

    overlap = sum(1 for labels in bucket_map.values() if len(labels) > 1)
    if overlap:
    ...
    if args.list:
        for b in ['growth', 'value']:
            names = sorted(t for t, labels in bucket_map.items() if b in labels)
            print(f"── {b.upper()} ──")
            print('  ' + '  '.join(names))
            print()
        names = sorted(t for t, labels in bucket_map.items() if not labels)
        print("── 미분류 ──")
        print('  ' + '  '.join(names))
        print()
        return

    # 스냅샷 CSV 저장 — 종목당 한 번만 (bucket 컬럼에 라벨 기록, 버킷별 파일 분할 안 함).
    # 예전엔 growth/value/cyclical/unclassified 4개 파일로 쪼갰는데, 다중 라벨 종목은
    # 같은 데이터가 여러 파일에 중복 저장되고 — 더 큰 문제로 — 종목의 "현재" 라벨 기준으로만
    # 파일을 정하다 보니 그 종목이 과거 다른 시점에 가졌던 라벨(PIT 기준)의 데이터가
    # 통째로 다른 파일에 묻혀 조회 불가능해지는 문제가 있었음(PIT 백테스트에서 발견).
    # bucket 컬럼이 이미 라벨을 다 담고 있으니 파일 분할 자체가 불필요.
    ...
    for i, ticker in enumerate(tickers_all, 1):
        labels = bucket_map.get(ticker, [])
        bucket_str = ','.join(labels)          # e.g. "growth,value", "growth", or "" (미분류)
        snap = build_snapshot(ticker, bucket_str, df_ttm, df_growth, df_val, df_px)
        if not snap.empty:
            snaps.append(snap)
        print(f"[{i:3}/{n}] {ticker:8} → {bucket_str or '(미분류)'}")

    print()
    merged = pd.concat(snaps, ignore_index=True)
    out_path = OUT_DIR / 'all_stocks.csv'
    merged.to_csv(out_path, index=False)
    print(f"  전체: {len(merged):,}행 → {out_path.relative_to(ROOT)}")
